...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['pricerange']) && isset($_GET['beds'])) {
    ...
    $no_table = $_GET['no_table'];
// Prepare and execute the SQL query to select hotels based on the filter criteria
$query = "SELECT * FROM restaurant WHERE priceperson <= ? AND no_table >= ?";

$stmt = $link->prepare($query);
$stmt->bind_param("ii", $pricerange, $no_table); // 'ii' indicates two integer parameters
$stmt->execute();
    ...
    <body>
        ...
        <h2>Hotel Filters</h2>
            <form id="filterForm" method="GET">
                ...
                <br><br>
                <label for="no_place">Number of Chair per Table:</label>
                <input type="number" id="no_place" name="no_table" min="1" max="5">
                <br><br>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="hotelModalLabel">Restaurant Details</h5>
...
function findHotels() {
    const pricerange = document.getElementById("pricerange").value;
    const no_place = document.getElementById("no_place").value;

    // Make an AJAX request to retrieve filtered hotels
    $.ajax({
        url: `../restaurantbooking/Restaurantbooking.php?pricerange=${pricerange}&beds=${no_table}`,
        method: 'GET',
        dataType: 'json', // Specify that you expect JSON data
        success: function (data) {
            // Parse the JSON response
            const resultsDiv = document.getElementById('results');
            resultsDiv.innerHTML = '<h3>Filtered Restaurant:</h3>';
            data.forEach(restaurant => {
                L.marker([restaurant.latitude, restaurant.longitude])
                    .addTo(map)
                    .bindPopup(`Restaurant: ${restaurant.restaurant_name}<br>Price: ${restaurant.priceperson}<br>Table: ${restaurant.no_table}`)
                    .on('click', function () {
                        // Display hotel details in the modal
                        const hotelDetails = document.getElementById('hotelDetails');
                        hotelDetails.innerHTML = `
                            <p><strong>Restaurant:</strong> ${restaurant.restaurant_name}</p>
                            <p><strong>Price:</strong> ${restaurant.priceperson}</p>
                            <p><strong>Table:</strong> ${restaurant.no_table}</p>
                        `;
                        // Show the modal
                        $('#hotelModal').modal('show');
                    });
                resultsDiv.innerHTML += `<p>Restaurant: ${restaurant.restaurant_name}, Price: ${restaurant.priceperson}, Table: ${restaurant.no_table}</p>`;
            });
        },
